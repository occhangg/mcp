...
"""Deny (reject) a TOOL_APPROVAL HITL task."""
...
async def deny_tool_approval(
    ...
    taskId: Annotated[str, Field(description='The TOOL_APPROVAL task identifier to deny')],
) -> Dict[str, Any]:
    """Deny a TOOL_APPROVAL task, preventing the agent from executing the requested tool.

    Use when the user has reviewed a pending TOOL_APPROVAL task and explicitly
    decided to reject it. The task must be in AWAITING_APPROVAL status. Denying
    sends action=REJECT to the FES API. After denial, the task transitions to
    SUBMITTED status and the agent reads the REJECT action to skip the tool.
    ...
    Returns on success: { action: "REJECT", taskId: str, result: { status: "SUBMITTED" } }
    ...
    """
    return await validate_and_submit(workspaceId, jobId, taskId, 'REJECT')
